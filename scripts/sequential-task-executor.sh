#!/bin/bash
# scripts/sequential-task-executor.sh
...
EOF
    ...
    # Send POST request
    response=$(curl -s -X POST -H "Content-Type: application/json" -d "$random_data" http://localhost:8080/api/v1/jobs/sample-1/execute)
